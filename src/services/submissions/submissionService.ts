...
export interface ISubmissionService {
  ...
  grade(submissionId: string, score: number, feedback: string): Promise<{ data: Submission }>;
}
...
export class SubmissionService implements ISubmissionService {
  ...
  async grade(submissionId: string, score: number, feedback: string): Promise<{ data: Submission }> {
    const { data } = await this.api.put(`/submissions/${submissionId}/grade`, { score, feedback });
    return data;
  }
}
